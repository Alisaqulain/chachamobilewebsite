...
import { ensureUnknownSupplier, UNKNOWN_SUPPLIER_NAME } from "@/lib/partsInventory";
...
export async function POST(request) {
  try {
    ...
    if (name.toLowerCase() === UNKNOWN_SUPPLIER_NAME.toLowerCase()) {
      return NextResponse.json({ error: "This name is reserved for the system default supplier" }, { status: 400 });
    }
    await connectDB();
    await ensureUnknownSupplier();
