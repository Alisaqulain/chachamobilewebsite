...
export default function SalesSystemLayout({ children }) {
  ...
  return (
    <div className="min-h-screen bg-zinc-100 lg:flex">
      ...
      {menuOpen ? (
        <aside className="fixed inset-y-0 left-0 z-40 w-64 border-r border-black/10 bg-zinc-950 text-white lg:hidden">
          {Sidebar}
        </aside>
      ) : null}

      <div className="min-w-0 flex-1 lg:ml-64">
        <header className="sticky top-0 z-20 flex items-center justify-between border-b border-black/10 bg-white px-4 py-3 shadow-sm lg:px-6">
          ...
          <Link href="/admin/website/dashboard" className="text-sm font-semibold text-brand-dim hover:underline">
            Website manager
          </Link>
        </header>
