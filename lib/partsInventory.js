...
/** Undo soldQty after a sale is deleted or reverted (mirror of increment). */
export async function revertSoldQtyForSaleLineItems(items) {
  if (!Array.isArray(items) || !items.length) return;
  for (const row of items) {
    const qty = Number(row.quantity || 0);
    if (!qty) continue;
    const gid = row.stockGroupId;
    if (gid && mongoose.Types.ObjectId.isValid(String(gid))) {
      await InventoryStockGroup.updateOne({ _id: gid }, { $inc: { soldQty: -qty } });
      continue;
    }
    const pid = row.productId;
    if (pid && mongoose.Types.ObjectId.isValid(String(pid))) {
      await InventoryStockGroup.updateOne({ linkedProductId: pid }, { $inc: { soldQty: -qty } });
    }
  }
}

/** After a sale is saved: bump soldQty on ledger lines (direct stock group and/or catalogue-linked group). */
