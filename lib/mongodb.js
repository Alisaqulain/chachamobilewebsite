...
async function runOnceMigrations() {
  ...
  try {
    try {
      const { migrateSalesLedgerCategoriesOnce } = await import("@/lib/migrateSalesLedgerCategories");
      await migrateSalesLedgerCategoriesOnce();
    } catch (e) {
      console.error("Sales ledger category migration:", e);
    }
