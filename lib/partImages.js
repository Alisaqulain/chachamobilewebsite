...
/** Local catalogue photos (shop hero + card fallback when DB has no image). */
export const SHOP_LOCAL_FALLBACK_IMAGES = [
  "/image1.jpg",
  "/image2.jpg",
  "/iamge3.jpg",
  "/iamge5.jpg",
...
/** Navbar / mega-menu / home tiles — local assets (reliable vs remote). */
export const CATEGORY_SLUG_CARD_IMAGE = {
  display: "/display.jpg",
  battery: "/battery.jpg",
  "charging-jack": "/charging.jpg",
  "folder-body": "/folder.jpg",
  speaker: "/speaker.png",
  camera: "/camera.jpg",
};
...
export function resolveProductCardImage(product) {
  ...
  const pool = SHOP_LOCAL_FALLBACK_IMAGES;
  const idx = hashMod(product?._id || product?.name || "x", pool.length);
