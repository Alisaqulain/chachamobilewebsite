...
export default function Navbar() {
  ...
  const { openInquiry } = useConversion();
  const isAdmin = pathname.startsWith("/admin");
  const navWaHref = buildWhatsAppUrl(
    "Hello Chacha Mobile — quick question from your website."
  );
        ...
        <nav className="mx-auto hidden h-full flex-1 items-center justify-center gap-1 lg:flex">
          ...
          {!isAdmin ? (
            <button
              type="button"
              onClick={openInquiry}
              className="rounded-full bg-brand px-5 py-2.5 text-sm font-bold text-black shadow-md transition hover:bg-brand-bright"
            >
              Get quote
            </button>
          ) : null}
              ...
              <div className="flex-1 overflow-y-auto overscroll-contain py-2">
                ...
                {!isAdmin ? (
                  <button
                    ...
                    className="block w-full min-h-12 px-5 py-3.5 text-left text-[15px] font-semibold text-brand-bright active:bg-white/10"
                  >
